...
function MerchandiseTransactionDetail() {
  ...
  const fetchProduct = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/merchandiseType/${productId}`);
      setProduct(res.data);
    } catch (error) {
      console.log(error);
    }
  ...
  const buyMerchandiseHandler = async () => {
    try {
      await axios.patch(`http://localhost:5000/merchandiseType/${productId}`, {
    ...
    <>
      ...
      <div className="flex flex-col pt-20 bg-black px-5 items-center">
        ...
        <div className="text-white">
          <button className="text-3xl text-red-100 border border-solid rounded-lg border-2 mb-40 mt-5" onClick={buyMerchandiseHandler}>
            Buy Now
          </button>
        </div>
      </div>
